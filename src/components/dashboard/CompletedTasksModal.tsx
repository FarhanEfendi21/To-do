    ...
    <>
      <div className="flex justify-center mt-2 mb-8">
        <button
          ...
          className="inline-flex items-center gap-2 text-[13px] font-medium text-muted-foreground hover:text-foreground transition-colors py-2 px-4 rounded-full bg-card border border-border shadow-sm hover:shadow-md"
        >
      ...
      {open && (
        ...
        >
          <div className="w-full sm:max-w-md bg-card rounded-t-[24px] sm:rounded-[24px] border border-border shadow-2xl overflow-hidden pb-4 sm:pb-0 animate-in slide-in-from-bottom-8 sm:slide-in-from-bottom-4 fade-in-20 duration-300 max-h-[85vh] flex flex-col">
            
            {/* Header */}
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-border shrink-0">
              <h2 className="text-base font-semibold text-foreground">Tugas Selesai</h2>
              <button
            ...
            <div className="overflow-y-auto px-6 py-5 flex flex-col gap-3">
              {tasks.map((task) => {
                ...
                return (
                  <div
                    key={task.id}
                    className="group bg-muted/50 rounded-[16px] px-5 py-4 flex gap-4"
                  >
                    ...
                    <div className="flex-1 min-w-0 pr-2">
                      <div className="flex flex-col items-start gap-1.5 object-top">
                        ...
                        <label
                          htmlFor={task.id}
                          className="block text-[15px] leading-snug cursor-pointer transition-colors line-through text-muted-foreground/60 font-normal"
                        >
                          {task.title}
                        </label>
                      </div>
                      ...
                      {task.description && (
                        <p className="text-[13px] leading-relaxed line-clamp-2 mt-1 mb-1 text-muted-foreground/40">
                          {task.description}
                        </p>
                      )}
                    </div>
